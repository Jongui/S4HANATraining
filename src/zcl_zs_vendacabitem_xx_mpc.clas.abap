class ZCL_ZS_VENDACABITEM_XX_MPC definition
  public
...
public section.
  ...
  types:
     TS_CABECALHO type ZVVENDACABXXX .
  types:
...
CLASS ZCL_ZS_VENDACABITEM_XX_MPC IMPLEMENTATION.
...
model->set_schema_namespace( 'ZS_VENDACABITEM_XXX_SRV' ).
...
lo_assoc_set = model->create_association_set( iv_association_set_name  = 'itemSet'                         "#EC NOTEXT
...
lo_entity_type->bind_structure( iv_structure_name   = 'ZVVENDACABXXX'
...
lo_property->/iwbep/if_mgw_odata_annotatabl~create_annotation( 'sap' )->add(
      EXPORTING
        iv_key      = 'unicode'
        iv_value    = 'false' ).
lo_property = lo_entity_type->create_property( iv_property_name = 'UnidMedida' iv_abap_fieldname = 'UNID_MEDIDA' ). "#EC NOTEXT
lo_property->set_type_edm_string( ).
...
lo_entity_type->bind_structure( iv_structure_name   = 'ZVVENDAITEMXXX'
                                iv_bind_conversions = 'X' ). "#EC NOTEXT
...
*&---------------------------------------------------------------------*
  ...
  CONSTANTS: lc_gen_date_time TYPE timestamp VALUE '20191218152659'.                  "#EC NOTEXT
